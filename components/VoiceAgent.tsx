            ...
            <div className="relative z-10 flex flex-col items-center justify-center flex-grow w-full max-w-4xl mx-auto py-8">
                ...
                <AnimatePresence mode="wait">
                    {callStatus === "idle" && !isRateLimited && (
                        ...
                        >
                            <div className="relative flex items-end bg-white/90 backdrop-blur-xl border border-white/50 rounded-[32px] px-2 pl-6 py-2 shadow-2xl shadow-brand-navy-900/5 focus-within:shadow-brand-navy-900/10 focus-within:bg-white focus-within:scale-[1.01] transition-all duration-300 ease-out">
                                {/* Text Input - Multiline textarea */}
                                <textarea
                                    placeholder={isChatMode ? "Reply to Doc..." : "Long day? Vent here..."}
                                    className="flex-1 bg-transparent outline-none text-brand-navy-900 placeholder-brand-navy-400/70 text-lg resize-none min-h-[56px] py-3.5 max-h-[160px] text-center placeholder:text-center font-medium my-auto"
                                ...
                                {confessionText.trim().length > 0 && (
                                    <motion.button
                                        ...
                                        disabled={isSubmittingConfession}
                                        className="p-3 my-1 ml-2 bg-brand-navy-600 text-white rounded-full hover:bg-brand-navy-700 transition-colors shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 flex-shrink-0"
                                        title="Send message"
                                    >
                                        {isSubmittingConfession ? (
                                ...
                                {!isChatMode && confessionText.trim().length === 0 && (
                                    <motion.button
                                        initial={{ opacity: 0, scale: 0.8 }}
                                        animate={{ opacity: 1, scale: 1 }}
                                        ...
                                        whileTap={{ scale: 0.95 }}
                                        onClick={startCall}
                                        className="p-3.5 my-1 ml-2 bg-gradient-to-br from-brand-navy-600 to-brand-navy-800 text-white rounded-full hover:shadow-brand-navy-600/25 transition-all shadow-lg hover:shadow-xl flex-shrink-0 group relative overflow-hidden"
                                        title="Talk to Doc"
                                    >
